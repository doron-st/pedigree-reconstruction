...
/**
 * An graph G = (V, E)
 * @inv all SimpleEdges are directed || all SimpleEdges are undirected
 */
public class SimpleGraph {
	private Map<SimpleVertex, List<SimpleVertex>> adj = new HashMap<SimpleVertex, List<SimpleVertex>>();
	private Map<String, SimpleVertex> nameMap = new HashMap<String, SimpleVertex>();
	private Map<String ,SimpleEdge> edges = new HashMap<String,SimpleEdge>();
	...
	/**
	 * adds a new undirected SimpleEdge to the graph
	...
	/**
	 * adds a new directed SimpleEdge to the graph
	...
	/**
	 * creates a SimpleVertex named <@param name> and adds it to te graph
	 ...
	 */
	public void createSimpleEdge(String str1, String str2) {
		createSimpleEdge(str1,str2,1);
	}
	/**
	 * Creates an undirected SimpleEdge that connects the SimpleVertex named str1 to the SimpleVertex named str2
	 * that means each SimpleVertex will add the other to its adjacency list
	 ...
	 * @param list: list of vertices with names corresponding to vertices in this graph
	 ...
	 * @return true iff a and b share a common vertex (by address)
	 */
	public static boolean graphsShareCommonVertex(SimpleGraph a, SimpleGraph b){
		List<SimpleVertex> aV = a.getVertices();
		Set<SimpleVertex> aVClone= new HashSet<SimpleVertex>();
		//clone a vertices
		for (SimpleVertex v : aV){
			aVClone.add(v);
		}
		aVClone.retainAll(b.getVertices());
		if(aVClone.size()==0){
			return false;
		}
		return true;
